...
{
    // Clase de prueba unitaria para la funcionalidad de la entidad Paises
    [TestClass]
    public class PaisesPruebaUnitaria
    {
        // Instancia de IPaisesRepositorio para interactuar con la base de datos a través del repositorio de países
        private IPaisesRepositorio? iRepositorio = null;

        // Instancia de la entidad Paises que se utilizará durante las pruebas
        private Paises? entidad = null;

        // Constructor de la clase de prueba unitaria
        public PaisesPruebaUnitaria()
        {
            // Crea una nueva conexión a la base de datos
            var conexion = new Conexion();

            // Establece la cadena de conexión a la base de datos SQL Server
            conexion.StringConnection = "server=localhost;database=db_Localizacion;uid=sa;pwd=12345;TrustServerCertificate=true;";

            // Inicializa el repositorio de países con la conexión creada
            iRepositorio = new PaisesRepositorio(conexion);
        }

        // Método que ejecuta todas las pruebas unitarias para la entidad Paises
        [TestMethod]
        ...
        {
            // Crea una nueva instancia de la entidad Paises con un nombre
            entidad = new Paises()
            {
                Nombre = "alemanisa" // Nombre del país
            };
